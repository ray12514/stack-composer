...
from stack_composer.model.package_set import spec_package_name
...
def test_rendered_workspace_exposes_lane_agnostic_packages(tmp_path: Path) -> None:
    ...
    assert not any(spec.startswith("openblas@") for spec in default_include)
    assert not any(spec.startswith("netlib-lapack@") for spec in default_include)
    assert any(spec.startswith("hdf5@") for spec in default_include)
    ...
    assert any(spec.startswith("python@") for spec in core_include)
    assert any(spec.startswith("py-numpy@") for spec in core_include)
    assert not any(
        spec_package in {"zlib", "xz", "zstd"}
        for spec_package in (spec.split("@", 1)[0] for spec in core_include)
    )
    ...
    assert any(spec.startswith("hdf5@") for spec in mpi_include)
    assert any(spec_package_name(spec) == "tau" for spec in mpi_include)
    # Another lane's explicit installs never leak in: the whitelist is the
    # lane's own roster, nothing else.
    assert not any(spec.startswith("openblas@") for spec in mpi_include)
